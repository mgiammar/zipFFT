// Microbenchmark for zipFFT's complex-to-complex (C2C/C2C) 2D padded convolution/cross-
// correlation.
//
...
//   nvcc -o bench_complex_conv2d benchmark/bench_complex_conv2d.cu \
...
//
// Run:
//   ./bench_complex_conv2d --warmup=20 --iters=100 --json=result.json
...
#include "../src/cuda/complex_conv_2d.cuh"
...
int main(int argc, char** argv) {
    ...
    if (args.has("help")) {
        std::cout << "Usage: bench_complex_conv2d [--warmup=N] [--iters=N] "
                     "[--repeats-per-sample=K] [--device=N] [--json=path] "
                     "[--peak-flops-tflops=X] [--peak-bandwidth-gbs=X]\n"
    ...
    std::cout << "========================================\n";
    std::cout << "zipFFT Complex (C2C/C2C) Conv2D Benchmark\n";
    std::cout << "========================================\n";
    ...
    // No Hermitian symmetry reduction -- both dimensions are full complex-to-complex.
    const size_t input_size = size_t(Batch) * SignalLengthY * SignalLengthX;
    const size_t workspace_size = size_t(Batch) * FFTSizeY * FFTSizeX;
    const size_t conv_size = size_t(FFTSizeX) * FFTSizeY;
    const size_t output_size = size_t(Batch) * ValidLengthY * ValidLengthX;

    MemoryFootprintMb mem{};
    mem.input = input_size * sizeof(float2) / (1024.0 * 1024.0);
    mem.workspace = workspace_size * sizeof(float2) / (1024.0 * 1024.0);
    mem.conv_data = conv_size * sizeof(float2) / (1024.0 * 1024.0);
    mem.output = output_size * sizeof(float2) / (1024.0 * 1024.0);
    ...
    float2* d_input = nullptr;
    float2* d_workspace = nullptr;
    float2* d_conv = nullptr;
    float2* d_output = nullptr;

    CUDA_CHECK_AND_EXIT(cudaMalloc(&d_input, input_size * sizeof(float2)));
    ...
    CUDA_CHECK_AND_EXIT(cudaMalloc(&d_output, output_size * sizeof(float2)));

    const int threads = 256;
    init_random_complex<<<(input_size + threads - 1) / threads, threads>>>(d_input, input_size,
                                                                           12345ULL);
    ...
    WorkTrafficPair work_traffic = complex_path::estimate_work_and_traffic(
