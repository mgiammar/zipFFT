...
class ComplexToComplexFFT1DConfig(BaseFFT1dConfig):
    ...
    def __init__(
        ...
        # output_data_type and signal_length are unused but kept for compatibility
        super().__init__(
    ...
    def get_output_data_type_assert(self) -> str:
        return ""  # Not used for C2C

    def get_signal_length_assert(self) -> str:
        return ""  # Not used for C2C

    def get_function_signature(
        self, include_pointer_type: bool = True, pointer_name: str = "data"
    ) -> str:
        cuda_type = type_str_to_cuda_type(self.input_data_type)
        fwd_inv_param = "true" if self.is_forward_fft else "false"

        if include_pointer_type:
            call = f"({cuda_type}* {pointer_name})"
        else:
            call = f"({pointer_name})"

        return (
            f"{self.function_name}<{cuda_type}, {self.fft_size}u, "
            f"{fwd_inv_param}, {self.elements_per_thread}u, {self.ffts_per_block}u>"
            f"{call}"
        )


class ComplexToComplexFFT1DGenerator(BaseFFT1dGenerator):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Set is_forward first so default_file_paths uses the correct prefix
        self.is_forward = is_forward

        # Filter configs for forward/inverse
        filtered = [cfg for cfg in (config_list or []) if cfg.is_forward_fft == is_forward]
    ...
    @classmethod
    def from_yaml(
        cls, yaml_path: str, config_class=BaseFFT1dConfig, is_forward=True, **kwargs
    ):
    ...
    @classmethod
    def from_json(
        cls, json_path: str, config_class=BaseFFT1dConfig, is_forward=True, **kwargs
    ):
    ...
    def default_file_paths(self) -> tuple[str, str, str]:
        ...
        return (
            str(self._gen_dir / f"{prefix}fft_c2c_1d_binding_cases.inc"),
            str(self._gen_dir / f"{prefix}fft_c2c_1d_implementations.inc"),
            str(self._gen_dir / f"{prefix}fft_c2c_1d_assertions.inc"),
        )

    def generate_static_assertions(self) -> str:
        # By default, generate for forward configs
        ...
        # Filter to only include non-empty conditions and unique
    ...
    def generate_binding_cases(self) -> str:
        ...
        cases = [
            cfg.get_binding_case_call(
                include_pointer_type=False, pointer_name="data_ptr"
            )
            for cfg in configs
        ]
...
if __name__ == "__main__":
    fwd_gen = ComplexToComplexFFT1DGenerator.from_yaml(
        yaml_path="configs/fft_c2c_1d.yaml",
        config_class=ComplexToComplexFFT1DConfig,
        is_forward=True,
    )
    inv_gen = ComplexToComplexFFT1DGenerator.from_yaml(
        yaml_path="configs/fft_c2c_1d.yaml",
        config_class=ComplexToComplexFFT1DConfig,
        is_forward=False,
    )
