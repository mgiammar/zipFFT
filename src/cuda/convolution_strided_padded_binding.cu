...
void dispatch_fft(float2* data, float2* kernel, unsigned int inner_batch_count, unsigned int outer_batch_count, int s) {
    block_convolution_strided_padded<float2, FFTSize, 8u, BatchSize>(data, kernel, inner_batch_count, outer_batch_count, s);
}
...
constexpr auto make_dispatch_table(std::index_sequence<Is...>) {
    return std::array<  
        std::pair<ComplexFFTConfig1D, std::function<void(float2*, float2*, unsigned int, unsigned int, int)>>,
        sizeof...(Is)>{
...
std::function<void(float2*, float2*, unsigned int, unsigned int, int)> get_fft_function(unsigned int fft_size,
...
void conv_c2c_1d_strided_padded(torch::Tensor input, torch::Tensor kernel, int s) {
    ...
    fft_func(data_ptr, kernel_ptr, inner_batch_count, outer_batch_count, s);
}
