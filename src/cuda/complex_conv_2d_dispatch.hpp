// Declaration of the per-config dispatch entry point for the padded complex-to-complex (C2C) 2D
// convolution/cross-correlation path.
//
// This header intentionally does NOT include complex_conv_2d.cuh (and therefore never pulls in
// cuFFTDx). complex_conv_2d_binding.cu includes only this declaration: it needs the function's
// *type* to build its compile-time dispatch table (taking the address of each
// dispatch_padded_complex_conv<...> specialization), but never calls or instantiates the body, so
// it never compiles a single cuFFTDx kernel itself.
//
// The definition lives in complex_conv_2d_dispatch_impl.cuh, which DOES include cuFFTDx and is
// in turn included only by the generated per-family shard .cu files under src/cuda/generated/
// (see setup.py's generate_conv_2d_shards()). Each shard provides the explicit instantiations
// for its slice of SUPPORTED_C2C_CONV_CONFIGS; the linker resolves the binding TU's
