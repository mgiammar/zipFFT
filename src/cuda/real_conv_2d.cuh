...
inline void padded_block_real_conv_2d_launcher(float* input_data, float2* fft_workspace,
    ...
    // Define IO handlers for each kernel. UseTiledSwizzledIO is only applied to the Y-dimension
    // (strided) handlers -- the X-dimension (contiguous) load/store is already coalesced and
    // doesn't benefit from tiling.
    // clang-format off
    ...
    // Tiled+swizzled Y I/O requires an even FFTs_per_block (see real_conv_2d_io.hpp for why) --
    // static_assert here for a clear top-level error message before it would otherwise surface
    // as an obscure failure deep in the IO handler.
...
template <typename ScalarType, typename ComplexType, unsigned int SignalLengthX,
          ...
          unsigned int FFTs_per_block_y = 0, bool UseTiledSwizzledIO = false>
int padded_block_real_conv_2d(ScalarType* input_data, ComplexType* fft_workspace,
    ...
    switch (arch) {
        ...
        case 800: padded_block_real_conv_2d_launcher<800, FFTSizeX, FFTSizeY, Batch, SignalLengthX, SignalLengthY, elements_per_thread_x, elements_per_thread_y, FFTs_per_block_x, FFTs_per_block_y, CrossCorrelate, UseTiledSwizzledIO>(input_data, fft_workspace, conv_data, output_data, device, stream); break;
#endif
#ifdef ENABLE_CUDA_ARCH_860
        case 860: padded_block_real_conv_2d_launcher<860, FFTSizeX, FFTSizeY, Batch, SignalLengthX, SignalLengthY, elements_per_thread_x, elements_per_thread_y, FFTs_per_block_x, FFTs_per_block_y, CrossCorrelate, UseTiledSwizzledIO>(input_data, fft_workspace, conv_data, output_data, device, stream); break;
#endif
#ifdef ENABLE_CUDA_ARCH_870
        case 870: padded_block_real_conv_2d_launcher<870, FFTSizeX, FFTSizeY, Batch, SignalLengthX, SignalLengthY, elements_per_thread_x, elements_per_thread_y, FFTs_per_block_x, FFTs_per_block_y, CrossCorrelate, UseTiledSwizzledIO>(input_data, fft_workspace, conv_data, output_data, device, stream); break;
#endif
#ifdef ENABLE_CUDA_ARCH_890
        case 890: padded_block_real_conv_2d_launcher<890, FFTSizeX, FFTSizeY, Batch, SignalLengthX, SignalLengthY, elements_per_thread_x, elements_per_thread_y, FFTs_per_block_x, FFTs_per_block_y, CrossCorrelate, UseTiledSwizzledIO>(input_data, fft_workspace, conv_data, output_data, device, stream); break;
#endif
        ...
        case 1030: padded_block_real_conv_2d_launcher<1030, FFTSizeX, FFTSizeY, Batch, SignalLengthX, SignalLengthY, elements_per_thread_x, elements_per_thread_y, FFTs_per_block_x, FFTs_per_block_y, CrossCorrelate, UseTiledSwizzledIO>(input_data, fft_workspace, conv_data, output_data, device, stream); break;
#endif
