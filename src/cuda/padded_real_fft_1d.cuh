...
inline void padded_block_real_fft_1d_launcher(Input_T* input_data, Output_T* output_data) {
    ...
    if constexpr (IsForwardFFT) {
        ...
        CUDA_CHECK_AND_EXIT(cudaFuncSetAttribute(padded_block_fft_r2c_1d_kernel<SignalLength, FFT>,
        ...
        padded_block_fft_r2c_1d_kernel<SignalLength, FFT>
            <<<1, FFT::block_dim, FFT::shared_memory_size>>>(input_data_t, output_data_t,
                                                             workspace);
    } else {
        // Increase shared memory size, if needed
        CUDA_CHECK_AND_EXIT(cudaFuncSetAttribute(padded_block_fft_c2r_1d_kernel<SignalLength, FFT>,
                                                 cudaFuncAttributeMaxDynamicSharedMemorySize,
                                                 FFT::shared_memory_size));
