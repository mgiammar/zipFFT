...
template <class FFT, class FFT_inv, int padding_ratio>
__launch_bounds__(FFT::max_threads_per_block) __global__
    ...
    load_strided_padded_smem<FFT, padding_ratio>(data, thread_data, shared_mem, inner_batch_count * FFT::ffts_per_block);
...
void conv_strided_padded_impl(torch::Tensor input, torch::Tensor kernel) {
...
PYBIND11_MODULE(conv_strided_padded, m) {  // First arg needs to match name in setup.py
    ...
    m.def("kernel_transpose", &conv_kernel_transpose_impl, "Transpose kernel for 1D C2C FFT convolution using cuFFTDx.");
    m.def("kernel_size", &conv_kernel_size_impl, "In-place 1D C2C FFT using cuFFTDx.");
