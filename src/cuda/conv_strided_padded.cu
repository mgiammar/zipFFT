...
template <class FFT, class FFT_inv, bool smem_transpose, bool read_kernel_transposed, int padding_ratio>
__launch_bounds__(FFT::max_threads_per_block) __global__
    ...
    load_strided_padded<FFT, smem_transpose, padding_ratio>(data, thread_data, shared_mem, inner_batch_count * FFT::ffts_per_block);
...
void do_convolution(struct FFTParams* fft_params, cudaStream_t strm) {
    ...
    // Increase shared memory size, if needed
    CUDA_CHECK_AND_EXIT(cudaFuncSetAttribute(
        conv_strided_padded_kernel<FFT, FFT_inv, smem_transpose, read_kernel_transposed, padding_ratio>,
...
unsigned int conv_strided_padded_impl(torch::Tensor input,
                                torch::Tensor kernel,
                                bool kernel_transpose,
                                bool read_kernel_transposed,
                                bool smem_transpose,
...
void conv_strided_func(torch::Tensor input, torch::Tensor kernel, bool read_kernel_transposed, bool smem_transpose) {
    conv_strided_padded_impl(input, kernel, false, read_kernel_transposed, smem_transpose, false);
}

void kernel_transpose_impl(torch::Tensor input, torch::Tensor kernel) {
    conv_strided_padded_impl(input, kernel, true, false, false, false);
}

unsigned int kernel_size_impl(torch::Tensor input) {
    return conv_strided_padded_impl(input, input, false, false, false, true);
}
...
PYBIND11_MODULE(conv_strided_padded, m) {  // First arg needs to match name in setup.py
    ...
    m.def("kernel_transpose", &kernel_transpose_impl, "Transpose kernel for 1D C2C FFT convolution using cuFFTDx.");
    m.def("kernel_size", &kernel_size_impl, "In-place 1D C2C FFT using cuFFTDx.");
