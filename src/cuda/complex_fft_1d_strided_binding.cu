...
void dispatch_fft(float2* data, unsigned int inner_batch_count, unsigned int outer_batch_count) {
    block_complex_fft_1d<float2, FFTSize, IsForwardFFT, 8u, BatchSize>(data, inner_batch_count, outer_batch_count);
}
...
constexpr auto make_dispatch_table(std::index_sequence<Is...>) {
    return std::array<
        std::pair<ComplexFFTConfig1D, std::function<void(float2*, unsigned int, unsigned int)>>,
        sizeof...(Is)>{
...
std::function<void(float2*, unsigned int, unsigned int)> get_fft_function(unsigned int fft_size,
