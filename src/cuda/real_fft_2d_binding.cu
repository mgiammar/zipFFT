...
struct RealFFTConfig2D {
    ...
    // Stride for complex data (always fft_size_x // 2 + 1)
    ...
    bool operator==(const RealFFTConfig2D& other) const {
        return fft_size_x == other.fft_size_x && fft_size_y == other.fft_size_y &&
               stride_y == other.stride_y && batch_size == other.batch_size &&
               is_forward == other.is_forward;
    }
};

// Define supported FFT configurations
// Format: (fft_size_y, fft_size_x, stride_y, batch_size, is_forward)
// Where: fft_size_y = H (rows), fft_size_x = W (columns), stride_y = fft_size_x // 2 + 1
static constexpr std::array<
    std::tuple<unsigned int, unsigned int, unsigned int, unsigned int, bool>, 36>
    SUPPORTED_FFT_CONFIGS = {{
        // Forward FFT configurations - input shape (H, W) -> output shape (H, W//2+1)
        {64, 64, 33, 1, true},     // 2D input (64, 64), 1 batch
        {64, 64, 33, 5, true},     // 2D input (64, 64), 5 batches
        {64, 64, 33, 10, true},    // 2D input (64, 64), 10 batches

        {128, 64, 33, 1, true},    // 2D input (128, 64), 1 batch
        {128, 64, 33, 5, true},    // 2D input (128, 64), 5 batches
        {128, 64, 33, 10, true},   // 2D input (128, 64), 10 batches

        {64, 128, 65, 1, true},    // 2D input (64, 128), 1 batch
        {64, 128, 65, 5, true},    // 2D input (64, 128), 5 batches
        {64, 128, 65, 10, true},   // 2D input (64, 128), 10 batches

        {128, 128, 65, 1, true},   // 2D input (128, 128), 1 batch
        {128, 128, 65, 5, true},   // 2D input (128, 128), 5 batches
        {128, 128, 65, 10, true},  // 2D input (128, 128), 10 batches

        {256, 256, 129, 1, true},  // 2D input (256, 256), 1 batch
        {256, 256, 129, 5, true},  // 2D input (256, 256), 5 batches
        {256, 256, 129, 10, true}, // 2D input (256, 256), 10 batches

        {512, 512, 257, 1, true},  // 2D input (512, 512), 1 batch
        {512, 512, 257, 5, true},  // 2D input (512, 512), 5 batches
        {512, 512, 257, 10, true}, // 2D input (512, 512), 10 batches

        // Inverse FFT configurations - input shape (H, W//2+1) -> output shape (H, W)
        {64, 64, 33, 1, false},    // 2D output (64, 64), 1 batch
        {64, 64, 33, 5, false},    // 2D output (64, 64), 5 batches
        {64, 64, 33, 10, false},   // 2D output (64, 64), 10 batches

        {128, 64, 33, 1, false},   // 2D output (128, 64), 1 batch
        {128, 64, 33, 5, false},   // 2D output (128, 64), 5 batches
        {128, 64, 33, 10, false},  // 2D output (128, 64), 10 batches

        {64, 128, 65, 1, false},   // 2D output (64, 128), 1 batch
        {64, 128, 65, 5, false},   // 2D output (64, 128), 5 batches
        {64, 128, 65, 10, false},  // 2D output (64, 128), 10 batches

        {128, 128, 65, 1, false},  // 2D output (128, 128), 1 batch
        {128, 128, 65, 5, false},  // 2D output (128, 128), 5 batches
        {128, 128, 65, 10, false}, // 2D output (128, 128), 10 batches

        {256, 256, 129, 1, false}, // 2D output (256, 256), 1 batch
        {256, 256, 129, 5, false}, // 2D output (256, 256), 5 batches
        {256, 256, 129, 10, false},// 2D output (256, 256), 10 batches

        {512, 512, 257, 1, false}, // 2D output (512, 512), 1 batch
        {512, 512, 257, 5, false}, // 2D output (512, 512), 5 batches
        {512, 512, 257, 10, false},// 2D output (512, 512), 10 batches
    }};

// Template dispatch functions for each supported configuration
// For forward FFT (real-to-complex)
template <unsigned int FFTSizeY, unsigned int FFTSizeX, unsigned int StrideY,
          unsigned int BatchSize, bool IsForwardFFT>
void dispatch_r2c_fft(float* input_data, float2* output_data, unsigned int batch_size) {
    ...
    block_real_fft_2d<float, float2, FFTSizeX, FFTSizeY, StrideY, true, elements_per_thread_x,
                      elements_per_thread_y, ffts_per_block_x, ffts_per_block_y>(
        input_data, output_data, batch_size);
}

// For inverse FFT (complex-to-real)
template <unsigned int FFTSizeY, unsigned int FFTSizeX, unsigned int StrideY,
          unsigned int BatchSize, bool IsForwardFFT>
void dispatch_c2r_fft(float2* input_data, float* output_data, unsigned int batch_size) {
    // Using optimal elements per thread and ffts per block based on FFT sizes
    constexpr unsigned int elements_per_thread_x = FFTSizeX <= 128 ? 8 : 16;
    constexpr unsigned int elements_per_thread_y = FFTSizeY <= 128 ? 8 : 16;
    constexpr unsigned int ffts_per_block_x = 1;
    ...
    block_real_fft_2d<float2, float, FFTSizeX, FFTSizeY, StrideY, false, elements_per_thread_x,
                      elements_per_thread_y, ffts_per_block_x, ffts_per_block_y>(
        input_data, output_data, batch_size);
}
...
constexpr auto make_r2c_dispatch_table(std::index_sequence<Is...>) {
    return std::array<
        std::pair<RealFFTConfig2D, std::function<void(float*, float2*, unsigned int)>>,
        sizeof...(Is)>{
        {{RealFFTConfig2D{
          ...
          []() {
              ...
              if constexpr (std::get<4>(config) == true) {  // Only include forward transforms
                  return dispatch_r2c_fft<std::get<0>(config), std::get<1>(config),
                                          std::get<2>(config), std::get<3>(config), true>;
              } else {
...
constexpr auto make_c2r_dispatch_table(std::index_sequence<Is...>) {
    return std::array<
        std::pair<RealFFTConfig2D, std::function<void(float2*, float*, unsigned int)>>,
        sizeof...(Is)>{
        {{RealFFTConfig2D{
              std::get<0>(SUPPORTED_FFT_CONFIGS[Is]), std::get<1>(SUPPORTED_FFT_CONFIGS[Is]),
              std::get<2>(SUPPORTED_FFT_CONFIGS[Is]), std::get<3>(SUPPORTED_FFT_CONFIGS[Is]),
          ...
          []() {
              constexpr auto config = SUPPORTED_FFT_CONFIGS[Is];
              if constexpr (std::get<4>(config) == false) {  // Only include inverse transforms
                  return dispatch_c2r_fft<std::get<0>(config), std::get<1>(config),
                                          std::get<2>(config), std::get<3>(config), false>;
              } else {
...
std::function<void(float*, float2*, unsigned int)> get_r2c_fft_function(unsigned int fft_size_y,
                                                                        unsigned int fft_size_x,
                                                                        unsigned int stride_y,
                                                                        unsigned int batch_size,
                                                                        bool is_forward) {
    ...
    for (const auto& entry : r2c_dispatch_table) {
        if (entry.first ==
                RealFFTConfig2D{fft_size_y, fft_size_x, stride_y, batch_size, is_forward} &&
...
std::function<void(float2*, float*, unsigned int)> get_c2r_fft_function(unsigned int fft_size_y,
                                                                        unsigned int fft_size_x,
                                                                        unsigned int stride_y,
                                                                        unsigned int batch_size,
                                                                        bool is_forward) {
...
void real_fft_r2c_2d_impl(torch::Tensor input, torch::Tensor output) {
    ...
    unsigned int fft_size_x, fft_size_y, stride_y, batch_size;
    ...
    // Stride is always fft_size_x // 2 + 1 for real FFTs
    ...
    auto fft_func = get_r2c_fft_function(fft_size_y, fft_size_x, stride_y, batch_size, true);
    TORCH_CHECK(fft_func != nullptr, "Unsupported FFT configuration: size_y=", fft_size_y,
                ", size_x=", fft_size_x, ", stride_y=", stride_y, ", batch=", batch_size,
                ", is_forward=true");
...
void real_fft_c2r_2d_impl(torch::Tensor input, torch::Tensor output) {
    ...
    if (input.dim() == 2) {  // input shape (H, W//2+1)
        fft_size_y = input.size(0);  // H - number of rows
        stride_y = input.size(1);    // W//2+1 - number of complex columns
        fft_size_x = (stride_y - 1) * 2;  // W - original size of real data
        batch_size = 1;
    } else if (input.dim() == 3) {  // input shape (batch, H, W//2+1)
        batch_size = input.size(0);  // batch size
        fft_size_y = input.size(1);  // H - number of rows
        stride_y = input.size(2);    // W//2+1 - number of complex columns
        fft_size_x = (stride_y - 1) * 2;  // W - original size of real data
    } else {
        TORCH_CHECK(false, "Input tensor must be 2D or 3D. Got ", input.dim(), "D.");
    }
    ...
    // Use the dispatch table to get the appropriate function
    auto fft_func = get_c2r_fft_function(fft_size_y, fft_size_x, stride_y, batch_size, false);
    TORCH_CHECK(fft_func != nullptr, "Unsupported FFT configuration: size_y=", fft_size_y,
                ", size_x=", fft_size_x, ", stride_y=", stride_y, ", batch=", batch_size,
                ", is_forward=false");
...
PYBIND11_MODULE(rfft2d, m) {  // First arg needs to match name in setup.py
    ...
    m.def("get_supported_fft_configs", &get_supported_fft_configs,
          "Get list of supported (fft_size_y, fft_size_x, stride_y, batch_size, is_forward) "
          "configurations");
}