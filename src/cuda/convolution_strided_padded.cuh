...
__launch_bounds__(FFT::max_threads_per_block) __global__
    ...
    if constexpr (!kernel_transpose) {
        ...
        example::io_strided<FFT>::load_transposed_kernel(kernel, kernel_thread_data);
...
template <unsigned int Arch, typename T, unsigned int FFTSize,
          ...
          unsigned int FFTs_per_block,
          bool kernel_transpose>
inline size_t block_convolution_transposed_kernel_size_getter(unsigned int inner_batch_count, unsigned int outer_batch_count) {
    using namespace cufftdx;
...
size_t block_convolution_transposed_kernel_size(unsigned int inner_batch_count,  unsigned int outer_batch_count) {
    auto arch = example::get_cuda_device_arch();
    ...
    /* clang-format off */
    switch (arch) {
        case 800: return block_convolution_transposed_kernel_size_getter<800, T, FFTSize, elements_per_thread, FFTs_per_block, kernel_transpose>(inner_batch_count, outer_batch_count);
        case 860: return block_convolution_transposed_kernel_size_getter<860, T, FFTSize, elements_per_thread, FFTs_per_block, kernel_transpose>(inner_batch_count, outer_batch_count);
        case 870: return block_convolution_transposed_kernel_size_getter<870, T, FFTSize, elements_per_thread, FFTs_per_block, kernel_transpose>(inner_batch_count, outer_batch_count);
        case 890: return block_convolution_transposed_kernel_size_getter<890, T, FFTSize, elements_per_thread, FFTs_per_block, kernel_transpose>(inner_batch_count, outer_batch_count);
        case 900: return block_convolution_transposed_kernel_size_getter<900, T, FFTSize, elements_per_thread, FFTs_per_block, kernel_transpose>(inner_batch_count, outer_batch_count);
        case 1200: return block_convolution_transposed_kernel_size_getter<900, T, FFTSize, elements_per_thread, FFTs_per_block, kernel_transpose>(inner_batch_count, outer_batch_count); 
        default:
            std::cerr << "Unsupported CUDA architecture: " << arch
                      << ". Supported architectures are 800, 860, 870, 890, "
                         "900, and 1200."
                      << std::endl;
            return 0;  // Error code for unsupported architecture
    }
